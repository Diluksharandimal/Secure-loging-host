...
app.post("/Asignin", (req, res) => {
    ...
    db.query(sql, [email], async (err, data) => {
        if (err) return res.status(500).json("Error");
        if (data.length === 0) return res.status(401).json("Admin not found");
...
app.get("/dashboard", authenticateToken, (req, res) => {
    if (req.user && req.user.email) {
        ...
        db.query(sql, [req.user.email], (err, data) => {
            if (err) return res.status(500).json("Error");
            if (data.length === 0) return res.status(403).json("Access Denied: You are not an admin");
...
app.listen(PORT, () => {
    ...
    // Automatically create an initial admin user
    createAdmin("admin@gmail.com", "Admin0011").catch(err => console.error("Admin creation error:", err));
});
